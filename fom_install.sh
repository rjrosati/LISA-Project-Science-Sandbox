...
usage="$(basename "$0") [-h] -- program to install needed packages

where:
    -h  show this help text

keyword argument options (given as key=value):
    env_name:  Name of generated conda environment. Default is 'nasa_fom_env'.
...
if [ -z ${env_name+x} ]; then env_name="nasa_fom_env"; fi

echo "Now installing into conda environment named $env_name."

echo "Installing NASA FOM setup."; 
...
"$pip_here" install corner eryn chainconsumer mpmath rich astropy;
